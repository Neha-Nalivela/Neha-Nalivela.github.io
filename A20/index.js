...
function Fetch() {
  const users = Value(); // get the users data
  ...
  if (user) {
    ...
    // Get and display user's posts
    const posts = post().filter(p => p.userId === user.id);
    const postHTML = posts.map(p => `<h3>${p.title}</h3><p>${p.body}</p>`).join("");
    document.getElementById("post-output").innerHTML = postHTML;
